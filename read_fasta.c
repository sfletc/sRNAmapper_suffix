 ...
 * Modified by Jason Jooste 23-10-2014
 */
	...
	while (fgets(ffp->buffer, FASTA_MAXLINE, ffp->fp)) {
		...
		for (s = ffp->buffer; *s != '\0'; s++) {
			...
			case ('A'):	case ('T'):	case ('G'):	case ('C'):	case ('N'):
				seq[n] = *s;
				break;
				//Non-canonical ambigous characters
			case ('K'):	case ('M'):	case ('R'):	case ('Y'):	case ('S'):	case ('W'):
			case ('B'):	case ('V'):	case ('H'):	case ('D'):	case ('X'):
				//Replace the character with out canonical way of representing an ambiguous base - 'N'
				seq[n] = 'N';
				break;
				//Newline is expected and can be safely ignored
			case ('\n'):
				continue;
				//Print an error if any unexpected characters come through
			default:
			...
			{ /* (remember, need space for the final '\0')*/
				//Jason: Altered this to double the array size instead of just adding the buffer size
				//- now the read time scales linearly
				nalloc *= 2;
				seq = realloc(seq, sizeof(char) * nalloc);
			}
		}
	}
	//Jason : If the character array is doubling in size it could be
	//left with a significant amount of empty space. Reallocate a final time
